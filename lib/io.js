...
/**
 * Simple IO implementation with async state
 *
 * NOTE: This is really just a simple implementation of
 * pub/sub with global state that can be selected
 */
function createIOWithAsyncState(reducer = () => {}) {
  /**
   * Keep subscribers in a list
   */
  const subscribers = [];
   ...
   * Method to select state
   */
  const getState = async (selector = (state) => state) => {
    return selector(state);
  };
...
module.exports = {
  createIO,
  createIOWithState,
  createIOWithStateTransitions,
  createIOWithAsyncState,
};
