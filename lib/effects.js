 ...
 */
module.exports.send = function describeSend(task, ref, message) {
  return {
    type: '@@send',
    id: uuid.v4(),
    task,
...
module.exports.send.resolve = function resolveSend({ task, ref, message }, io, engine, parentTask, cb) {
  task && task.isRunning && task.messages.put([message, ref, parentTask]);
  parentTask.isRunning && cb();
};
