...
function createEffectsResolver(effects, monitor) {
  ...
  return function(interpreter, context, io, task) {
    ...
    return function resolveEffects(
       ...
       */
      monitor && monitor.effectTriggered && monitor.effectTriggered(getSerializableEffect(effect), getSerializableTask(parent));
      ...
      return resolve(effect, io, engine, task, (err, result) => {
        if (err) {
           ...
           * NOTE: Monitoring event
           */
          monitor && monitor.effectRejected && monitor.effectRejected(getSerializableEffect(effect), err);
           ...
           * NOTE: Monitoring event
           */
          monitor && monitor.effectResolved && monitor.effectResolved(getSerializableEffect(effect), result);
...
function createInterpreter(
  ...
  middleware = [],
  monitor = {
    effectTriggered: (effect, parent) => console.log('effect triggered', effect, parent),
  ...
  return function interpreter(
    ...
    const task = {
      ...
      cancel: () => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.taskCancelled && monitor.taskCancelled(getSerializableTask(task));
      ...
      attachFork: (taskToAttach) => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.taskAttached && monitor.taskAttached(getSerializableTask(task), getSerializableTask(taskToAttach));
      },
      detachFork: (taskToDetach) => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.taskDetached && monitor.taskDetached(getSerializableTask(task), getSerializableTask(taskToAttach));
      },
      attachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.effectAttached && monitor.effectAttached(getSerializableEffect(effect), getSerializableTask(task));
      },
      detachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.effectDetached && monitor.effectDetached(getSerializableEffect(effect), getSerializableTask(task));
      },
      /**
       * Task context
       */
     ...
     * NOTE: Monitoring event
     */
    monitor && monitor.taskCreated && monitor.taskCreated(getSerializableTask(task));
    ...
    function resolveYieldRecursively(state) {
      ...
      if (state.done) {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor.taskSucceeded && monitor.taskSucceeded(getSerializableTask(task), task.result);
