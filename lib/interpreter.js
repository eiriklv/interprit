    ...
    function resolveYieldRecursively(state) {
      ...
      resolveYield(applyMiddleware(state.value), function (err, result) {
        ...
        if (err) {
           ...
           * if so, we reject the task and recursively handle
           * the resolving / finishing
           */
          let nextState;
          ...
          try {
            ...
            /**
             * Update task state if the throwing of the error crashes the generator
             */
            task.error = error;

            /**
             * Step the generator to get a valid state
             */
            nextState = iterator.next();
          }

          /**
           * Continue the recursive yielding
           */
          return resolveYieldRecursively(nextState);
        }
