  ...
  return function(interpreter, context, io, task) {
    ...
    return function resolveEffects(
      ...
      if (!isEffect(effect)) {
         ...
         * NOTE: Monitoring event
         */
        monitor && monitor(effectResolved(effect, effect));
  ...
  return function interpreter(
    ...
    const task = {
      ...
      detachFork: (taskToDetach) => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskDetached(task, taskToDetach));
       ...
       */
      terminateIfReady: () => {
        if (!taskCompleted || task.attachedForks.length) {
          return;
        }
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskTerminated(task, task.error, task.result));
    ...
    function resolveYieldRecursively(state) {
      ...
      if (state.done) {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskCompleted(task));
