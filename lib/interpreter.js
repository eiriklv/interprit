  ...
  return function interpreter(
    ...
    const task = {
      ...
      cancel: () => {
         ...
         * NOTE: Monitor event
         */
        monitor.taskCancelled(getSerializableTask(task));
      ...
      attachFork: (taskToAttach) => {
         ...
         * NOTE: Monitor event
         */
        monitor.taskAttached(getSerializableTask(task), getSerializableTask(taskToAttach));
      },
      detachFork: (taskToDetach) => {
         ...
         * NOTE: Monitor event
         */
        monitor.taskDetached(getSerializableTask(task), getSerializableTask(taskToAttach));
      },
      attachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor.effectAttached(getSerializableEffect(effect), getSerializableTask(task));
      },
      detachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor.effectDetached(getSerializableEffect(effect), getSerializableTask(task));
      },
      /**
       * Task context
       */
     ...
     */
    monitor.taskCreated(getSerializableTask(task));
    ...
    function resolveYieldRecursively(state) {
      ...
      if (state.done) {
         ...
         * NOTE: Monitor event
         */
        monitor.taskSucceeded(getSerializableTask(task), task.result);
       ...
       * NOTE: Monitor event
       */
      monitor.effectTriggered(getSerializableEffect(state.value), getSerializableTask(task));
      ...
      resolveYield(applyMiddleware(state.value), function (err, result) {
        ...
        if (err) {
           ...
           * NOTE: Monitor event
           */
          monitor.effectRejected(getSerializableEffect(state.value), err);
           ...
           * NOTE: Monitor event
           */
          monitor.taskFailed(getSerializableTask(task), reason);
         ...
         * NOTE: Monitor event
         */
        monitor.effectResolved(getSerializableEffect(state.value), result);
