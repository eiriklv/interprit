...
function createEffectsResolver(effects, monitor) {
  ...
  return function(interpreter, context, io, task) {
    ...
    return function resolveEffects(
       ...
       */
      monitor && monitor(effectTriggered(getSerializableEffect(effect), getSerializableTask(parent)));
      ...
      return resolve(effect, io, engine, task, (err, result) => {
        if (err) {
           ...
           * NOTE: Monitoring event
           */
          monitor && monitor(effectRejected(getSerializableEffect(effect), err));
           ...
           * NOTE: Monitoring event
           */
          monitor && monitor(effectResolved(getSerializableEffect(effect), result));
  ...
  return function interpreter(
    ...
    const task = {
      ...
      cancel: () => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskCancelled(getSerializableTask(task)));
      ...
      attachFork: (taskToAttach) => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskAttached(getSerializableTask(task), getSerializableTask(taskToAttach)));
      },
      detachFork: (taskToDetach) => {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskDetached(getSerializableTask(task), getSerializableTask(taskToAttach)));
      },
      attachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(effectAttached(getSerializableEffect(effect), getSerializableTask(task)));
      },
      detachEffect: (effect) => {
        /**
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(effectDetached(getSerializableEffect(effect), getSerializableTask(task)));
      },
      /**
       * Task context
       */
     ...
     * NOTE: Monitoring event
     */
    monitor && monitor(taskCreated(getSerializableTask(task)));
    ...
    function resolveYieldRecursively(state) {
      ...
      if (state.done) {
         ...
         * NOTE: Monitor event
         */
        monitor && monitor(taskSucceeded(getSerializableTask(task), task.result));
