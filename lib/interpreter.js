    ...
    const task = {
      ...
      terminateIfReady: () => {
        if (!task.isCompleted || task.attachedForks.length) {
          console.log(`* task ${task.type} not ready for termination`);
          return;
        }

        if (task.isTerminated) {
          console.log(`* task ${task.type} already terminated`);
          return;
        }

        console.log(`* task ${task.type} terminating`);
        ...
        if (task.error) {
          ...
          console.log(`resolving task ${task.type}:${task.id}`);
