    ...
    function resolveYieldRecursively(state) {
      ...
      resolveYield(applyMiddleware(state.value), function (err, result) {
        ...
        if (err) {
           ...
           * and continue
           *
           * TODO: Check if this is actually the right way to
           * do it. What happens when you throw something in
           * there and it is caught? The generator is not done,
           * so maybe it should not get the status of not running
           * and be rejected with an error?
           */
