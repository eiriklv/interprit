 ...
 * TODO: Avoid using local state, and instead use remote services
 * to store the events in your own "local" async state, and then
 * be able to query it (both ES and CQRS)
 *
 * const oldState = yield call(getProjection, eventId - 1 || undefined); // get a projection for a specific event id or just the current one
 * yield call(commitEventToLocalStore, event); // this should probably be done by the command handler / process
 * const newState = yield call(getProjection, eventId) // get a projection for specific event id
 */
